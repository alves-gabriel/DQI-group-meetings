...
- The archive page includes client-side filters for text search, status, year, and clicked tags. Search covers titles, speakers, and tags. Archive tag pills are buttons that toggle an exact tag filter and combine with the other controls. Filter behavior lives in `assets/js/archive-filters.js` and uses `data-*` attributes rendered by `archive.html`.
